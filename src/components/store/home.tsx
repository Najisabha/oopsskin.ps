...
export function HomeContent({ products }: { products: Product[] }) {
  ...
  return <>
    ...
    <div className="border-b bg-background"><div className="page-container grid grid-cols-2 gap-5 py-6 lg:grid-cols-4">{[[Heart,"Chosen with care","مختارة بعناية"],[ShoppingBag,"Shop with or without an account","تسوقي بحساب أو بدونه"],[Package,"Cash on delivery","الدفع عند الاستلام"],[Truck,"Delivered to your doorstep","توصيل لباب بيتك"]].map(([Icon,en,ar],i) => { const I = Icon as typeof Heart; return <div key={i} className="flex items-center justify-center gap-2.5 text-[11px] sm:text-xs"><I className="size-4 shrink-0 text-primary" /><span>{t(en as string,ar as string)}</span></div>; })}</div></div>
