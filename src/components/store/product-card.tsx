...
export function ProductCard({ product }: { product: Product }) {
  ...
  return <article className="group min-w-0">
    <div className="relative aspect-[4/5] overflow-hidden rounded-xl bg-muted">
      <Link href={`/product/${encodeURIComponent(product.id)}`} className="absolute inset-0" aria-label={t(product.name,product.nameAr)}><ProductImage src={product.images[0]} alt={t(product.name,product.nameAr)} className="transition-transform duration-700 group-hover:scale-105" /></Link>
