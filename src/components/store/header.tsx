...
export const navigation = [ ["/", "Home", "الرئيسية"], ["/best-sellers", "Best sellers", "الأكثر مبيعاً"], ["/new", "New arrivals", "وصل حديثاً"], ["/makeup", "Makeup", "المكياج"], ["/skincare", "Skincare", "العناية بالبشرة"], ["/packages", "Habibti kits", "مجموعات حبيبتي"], ["/all-products", "Shop all", "كل المنتجات"] ];
export function Header() {
  ...
  return <>
    ...
    <header className="sticky top-0 z-40 border-b bg-background/95 backdrop-blur-xl">
      ...
      <nav aria-label={t("Main navigation", "القائمة الرئيسية")} className="hidden h-12 items-center justify-center gap-9 border-t border-border/60 lg:flex">{navigation.map(([href,en,ar]) => <Link key={href} href={href} className={cn("relative flex h-full items-center text-xs font-medium transition-colors hover:text-primary after:absolute after:inset-x-0 after:bottom-0 after:h-0.5 after:bg-primary", pathname === href ? "text-primary" : "after:hidden")}>{t(en,ar)}</Link>)}<Link className="flex items-center gap-1 text-xs text-muted-foreground" href="/about">{t("Our story", "من نحن")}<ArrowUpRight className="size-3.5" /></Link></nav>
    </header>
