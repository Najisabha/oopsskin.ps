...
export const profileSchema = z.object({ name: text(100), phone: z.string().trim().max(30).default(""), address: z.string().trim().max(500).default(""), city: z.union([city, z.literal("")]).default("") });
...
export const checkoutSchema = z.object({ name: text(100), email: z.email().max(254), phone: z.string().trim().regex(/^[+\d\s()-]{7,25}$/, "Enter a valid phone number"), address: text(500), city, notes: z.string().max(1000).default(""), requestKey: z.uuid() });
