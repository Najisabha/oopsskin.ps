export async function api<T>(path: string, method = "GET", body?: unknown): Promise<T> {
  ...
  if (!response.ok) throw new Error((typeof document !== "undefined" && document.documentElement.lang === "ar" ? data.messageAr : null) || data.message || "Something went wrong. Please try again.");
  return data as T;
}
